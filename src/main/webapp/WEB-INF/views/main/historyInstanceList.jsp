...
<script type="text/javascript">
	...
	function getTaskHistory(data) {
		...
		$("#demoGrid").jqGrid({
             ...
             rowNum: 10,
             rowList: [10],
             colNames:['任务名称', '负责人', '流程实例ID','流程定义ID'],
             colModel:[
                 {name:'name',index:'name', width:60, align:"center", sorttype:"int",search:true},
                 {name:'assignee',index:'assignee', width:120},
                 {name:'processInstanceId',index:'processInstanceId', width:100},
                 {name:'processDefinitionId',index:'processDefinitionId', width:80}
             ],
             pager: "#gridPager",
             viewrecords: true,
             hidegrid:true,
	...
	function getProcessHistory(data) {
		...
		$("#demoGrid").jqGrid({
             ...
             rowNum: 10,
             rowList: [10],
             colNames:['流程实例ID','流程定义ID','流程开始人', '结束节点' ],
             colModel:[
             	 {name:'processInstanceId',index:'processInstanceId', width:100},
                 {name:'processDefinitionId',index:'processDefinitionId', width:80},
                 {name:'startUserId',index:'startUserId', width:60, align:"center", sorttype:"int",search:true},
                 {name:'endActivityId',index:'endActivityId', width:120}
             ],
             pager: "#gridPager",
             viewrecords: true,
             hidegrid:true,
         ...
         $('#tab-home').click(function() {
         	$.ajax({
				...
				type: "GET",
				url: "${ctx}/history/taskList", 
				dataType:'json',
				success: function(data) {
					console.log("-------------="+data.data);
					getTaskHistory(data.data);
        ...
        $('#tab-sec').click(function() {
         	$.ajax({
				contentType: "application/json",
				type: "GET",
				url: "${ctx}/history/processList",
				dataType:'json',
				success: function(data) {
					console.log(data);
					getProcessHistory(data.data);
					jQuery("#demoGrid").clearGridData();
					jQuery("#demoGrid").jqGrid('setGridParam',{data:data.data}).trigger('reloadGrid');
				},
				error: function(m) {
              ...
              <div class="b-view genere jfk-scrollbar" id="genere">
                <div class="minheight-forfooter">
                  <ul class="b-list-3" id="aside-menu">
                    <li class="b-list-item"><a class="sprite2 on" hideFocus="true" href="javascript:;" id="tab-home" unselectable="on"><span class="text1"><span class="img-ico aside-disk"></span>任务历史</span></a></li>
                    <li class="b-list-item separator-1"></li>
                    <li class="b-list-item"><a cat="2" id="tab-sec" class="b-no-ln type-a-pic" hideFocus="true" href="javascript:;"><span class="text1"><span class="img-ico aside-mpic"></span>流程历史</span></a></li>
                  </ul>           
                </div>              
              </div>
            </div>
