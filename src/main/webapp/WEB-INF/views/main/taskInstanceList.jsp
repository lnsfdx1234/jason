...
function getProcessInstances(data) {
		$("#demoGrid").jqGrid({
             ...
             rowNum: 10,
             rowList: [10],
             colNames:['任务ID', '任务名称', '任务所有人', '描述','任务期限','分配人','执行ID','父节点', '流程实例ID'],
             colModel:[
                 ...
                 {name:'executionId',index:'executionId', width:150},
                 {name:'parentTaskId',index:'parentTaskId', width:150},
	...
	$(function() {
        ...
        $('#tab-home').click(function() {
        	...
        	jQuery("#demoGrid").clearGridData();
         	$.ajax({
				...
				type: "GET",
				url: "${ctx}/service/process-instances", 
				success: function(data) {
					...
					$.each(data.data, function(i, item){
						...
						$.ajax({
							contentType: "application/json",
							type: "GET",
							url: "${ctx}/service/process-instance/"+item.id+"/tasks", 
							success: function(obj) {
								for(var j=0;j<obj.data.length;j++){
									list.push(obj.data[j]);
								}
							 	console.log('111----+==----');
					  			console.log(list);
					  			console.log('----+==----');
					  			count--;
					  			if( count==0 ){
								  	getProcessInstances(list);
								  	jQuery("#demoGrid").jqGrid('setGridParam',{data:list}).trigger('reloadGrid');
					  			}
							},
							error: function(m) {
								console.log(11);
        ...
        $('#tab-sec').click(function() {
         	$.ajax({
				contentType: "application/json",
				type: "GET",
				url: "${ctx}/service/tasks?assignee=leaderuser&start=0&size=10&sort=id&order=asc", 
				success: function(data) {
					console.log(data);
              ...
              <div class="b-view genere jfk-scrollbar" id="genere">
                <div class="minheight-forfooter">
                  <ul class="b-list-3" id="aside-menu">
                    <li class="b-list-item"><a class="sprite2 on" hideFocus="true" href="javascript:;" id="tab-home" unselectable="on"><span class="text1"><span class="img-ico aside-disk"></span>信贷任务实例</span></a></li>
                    <li class="b-list-item separator-1"></li>
                    <li class="b-list-item"><a cat="2" class="b-no-ln type-a-pic" hideFocus="true" href="javascript:;"><span class="text1"><span class="img-ico aside-mpic"></span>风险任务实例</span></a></li>
                  </ul>           
                </div>              
              </div>
            </div>
            ...
            <section class="fns">
              ...
              <div class="b-rlv share-line clearfix">
				...
				<section class="files">
                  ...
                  <form action="javascript:void(0)" class="file-list b-rlv" id="fileList" method="get" name="fileList">
